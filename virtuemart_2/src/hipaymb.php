...
/**
 *
 * payments using Hipay atm payment reference:
 * @author Diogo Ferreira
 * @version $Id: hipaymb.php 1111 2015-11-30 21:55:54Z $
 * @package VirtueMart
 * @subpackage payment
...
class plgVmPaymentHipaymb extends vmPSPlugin {
	...
	public function getVmPluginCreateTableSQL () {
		return $this->createTableSQL ('Payment Hipay CompraFacil ATM');
	}
	...
	function plgVmConfirmedOrder ($cart, $order) {
		...
		if ($account["cf_entity"] == "11249") $webservice_url .= "2";
		...
		$origin = "test";
		$username = $account["cf_username"];
		$password = $account["cf_password"];
		...
		$IDUserBackoffice = $dbValues['order_number']; //-1 in most cases
		$timeLimitDays = $account["cf_days"]; //3, 30 or 90 days; only used for entity 11249 and ignored for entity 10241 (must be 0)
	    ...
	    error_log($origin,3,"error_log.log");
		...
		try 
		{
			...
			if ($res->getReferenceMBResult)
			{
				...
				$html = '<table cellpadding="6" cellspacing="2" style="width: 350px; height: 55px; margin: 10px 0 2px 0;border: 1px solid #ddd"><tr>
					...
					<tr>
						<td rowspan="3" style="width:110px;padding: 0px 5px 0px 5px;vertical-align: middle;"><img src="plugins/vmpayment/hipaymb/hipaymb/assets/images/multibanco.jpg" style="margin-bottom: 0px; margin-right: 0px;"></td>
	...
	function plgVmOnShowOrderBEPayment ($virtuemart_order_id, $virtuemart_payment_id) {
		...
		$html .= $this->getHtmlRowBE ('HIPAYMB_TITLE', $paymentTable->payment_name);
		...
		$html .= $this->getHtmlRowBE ('HIPAYMB_VALIDITY', $payment_details[0]->days);
